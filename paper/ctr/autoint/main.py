...
from paper.ctr.autoint.dataset import AutoIntDataset
from paper.ctr.autoint.model import AutoInt


class AutoIntController:
    ...
    def __init__(self):
        ...
        self.train_dataset = AutoIntDataset(train_df.iloc[:, 15:].values.tolist(),
                                            train_df.iloc[:, 2:15].values.tolist(),
                                            train_df.iloc[:, 1].values.tolist())

        self.test_dataset = AutoIntDataset(test_df.iloc[:, 15:].values.tolist(),
                                           test_df.iloc[:, 2:15].values.tolist(),
                                           test_df.iloc[:, 1].values.tolist())

        self.model = AutoInt(list(sparse_feat_grp_info.values()),
